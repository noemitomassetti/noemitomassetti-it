          ...
          {getBlogPosts()
            ...
            <Card key={post.id} className="flex flex-col h-full bg-card border-border/50 hover:border-primary/50 transition-colors overflow-hidden">
                <div className="h-48 w-full bg-muted overflow-hidden">
                  <img src={post.image} alt={post.alt || `Copertina articolo: ${post.title}`} loading={index < 2 ? "eager" : "lazy"} decoding="async" width="600" height="300" className="w-full h-full object-contain bg-background opacity-90 transition-transform duration-500 hover:scale-105" />
                </div>
